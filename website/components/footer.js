    ...
    <div className="md:flex md:justify-between container max-w-8xl mx-auto">
        <div className="mb-6 md:mb-0">
            <a href="/" className="flex items-center">
                <img src={config.devent.logo} className="mr-3 h-8" alt={config.devent.name} />
                <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">{config.devent.name}</span>
            </a>
        </div>
        <div className="grid grid-cols-2 gap-8 sm:gap-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <ul className="text-gray-600 dark:text-gray-400">
                    { Object.keys(config.footer.legal).map((k) => (
                        <li className="mb-4">
                            <a href={config.footer.links[k]} className="hover:underline">{k}</a>
                        </li>
                    ))}
